...
#define CHANNEL 1
uint8_t update_recv_data;
bool update_flag;
uint8_t joy_value[3]; //JOY_X,JOY_Y,CLICK(PULL-DOWN)
...
//  Serial.println("");
  update_recv_data = *data;
  if(update_flag != 1)
  {
    if(update_recv_data == 255) buf_index=0;
    else if(update_recv_data == 254) buf_index++;
    else if(update_recv_data == 253) buf_index++;

    else joy_value[buf_index] = update_recv_data;
