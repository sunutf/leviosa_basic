...
//Wand Part
// warning : you can't use A0/A1 (ADC2) when wifi is on. use A2/A3(ADC1)
...
void readJoyData(void){
  joy_x = analogRead(JOY_X)/100;
  joy_y = analogRead(JOY_Y)/100;
...
void sendData(uint8_t *data) {
  ...
  for (int i = 0; i < SlaveCnt; i++) {
    ...
    if(data == &joy_x) inst_data = 255;
    else if(data == &joy_y) inst_data = 254;
    else if(data == &button) inst_data = 253;
    esp_now_send(peer_addr, &inst_data, sizeof(inst_data));
